...
public class AnimaisDAO {
    ...
    private final String kSelectAllSql = "SELCET * from "+ kTableName +";";

    private final String selectName(String nome){
        return "SELCET * from "+ kTableName +" WHERE 'nome'= "+nome+";";
    ...
    public List<Animal> getAll() {
        ...
        try {
            Statement statement = conn.createStatement();
            ResultSet rs = statement.executeQuery(selectName(kSelectAllSql));
            while (rs.next()) {
                Animal animal = new Animal(rs.getInt("id"));
                animal.nome = rs.getString("nome");
                animal.tipo = rs.getString("tipo");
